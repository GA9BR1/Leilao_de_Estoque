...
describe 'Usuário tenta criar uma nova dúvida', selenium: true do
  it 'e consegue com sucesso' do 
    ...
    expect(Doubt.all.count).to eq(1)
    expect(page).to have_content(Doubt.find(1).content)
  end

  it 'e falha por deixar dúvida em branco' do
    video_card_category = ItemCategory.create!(name: 'Placas de Vídeo')
    ...
    expect(Doubt.all.count).to eq(0)
  ...
  it 'e consegue com sucesso(ADM)' do 
    ...
    click_on "Leilões em andamento"
    click_on batch.code
    fill_in 'Dúvida', with: 'Pessoal, o lote pode ser retirado presencialmente no endereço XPTO'
    click_on 'Enviar'
    expect(page).to have_content("#{user.name} (Administrador)")
    expect(Doubt.all.count).to eq(1)
    expect(page).to have_content(Doubt.find(1).content)
  end

  it 'e falha por deixar dúvida em branco(ADM)' do 
    video_card_category = ItemCategory.create!(name: 'Placas de Vídeo')
    ...
    click_on batch.code
    click_on 'Enviar'
    expect(page).not_to have_content("#{user.name} (Administrador)")
    expect(Doubt.all.count).to eq(0)
    expect(page).to have_content('Dúvida não pode ficar em branco')
  end
end