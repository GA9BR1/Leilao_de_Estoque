...
describe 'Usuário tenta criar uma request de dúvida' do
  it 'com sucesso' do
    ...
    post(batch_doubts_path(batch.id), params: {doubt: {batch_id: batch.id, user_id: user3.id, content: 'Olá onde é feita a retirada do lote ?'}})

    expect(Doubt.all.count).to eq(1)
    expect(Doubt.find(1).content).to eq('Olá onde é feita a retirada do lote ?')
  end
  ...
  it 'e o leilão já acabou e não é o ganhador' do
    ...
    post(batch_doubts_path(batch.id), params: {doubt: {batch_id: batch.id, user_id: user3.id, content: 'ROUBADO DEMAIS !'}})
  ...
  it 'e falha ao tentar criar uma dúvida em nome de outro usuário' do
    video_card_category = ItemCategory.create!(name: 'Placas de Vídeo')
    ...
    batch = Batch.create!(start_date: 3.day.ago, end_date: 7.day.from_now, minimum_bid_difference: 30, created_by_id: user.id, minimum_bid: 30)
    BatchItem.create!(batch_id: batch.id, item_id: item.id)
    batch.approved_by = user2
    batch.save!
    login_as(user4)
